...
const countClassStatsByParticipant = () => {
  ...
  classes.forEach(
    ({
      ...
      if (status === "Held") {
        ...
        if (isLate) {
          summary[name].lateCancelled[typeKey] +=
            classCancellation?.durationCancelled || 0;
        } else {
          summary[name].cancelled[typeKey] +=
            classCancellation?.durationCancelled || 0;
        }
  ...
  // print amount earned assuming each class is paid at 62000 per hour for in-person and 50000 per hour for virtual; late cancellations earn 75%
  let amountEarned = 0;
  let virtualWorked = 0;
  let inPersonWorked = 0;
  for (const name in summary) {
    const { held, lateCancelled } = summary[name];
    const lateFactor = 0.75;
    virtualWorked += held.virtual;
    inPersonWorked += held.inPerson;
    amountEarned += held.inPerson * 62000 + held.virtual * 50000;
    amountEarned +=
      (lateCancelled.inPerson * 62000 + lateCancelled.virtual * 50000) *
      lateFactor;
  }
  console.log(
    `Total hours worked: ${(virtualWorked + inPersonWorked).toLocaleString()} hours`,
  );
  console.log(`- Virtual: ${virtualWorked.toLocaleString()} hours`);
  console.log(`- In-person: ${inPersonWorked.toLocaleString()} hours`);
  console.log(
    `Total amount earned (late cancellations at 75%): ${amountEarned.toLocaleString()} COP`,
  );
